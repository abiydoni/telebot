...
try {
    // Ambil hari dan bulan hari ini
    $todayDay = date('d');
    $todayMonth = date('m');

    // Query warga yang ulang tahun hari ini, kecualikan yang sudah meninggal
    $stmt = $pdo->prepare("SELECT name AS nama, tanggalLahir AS tgl_lahir FROM users WHERE SUBSTRING(tanggalLahir, 1, 2) = ? AND SUBSTRING(tanggalLahir, 4, 2) = ? AND statusHidup = 'Aktif' AND villageId = 'village_001'");
    $stmt->execute([$todayDay, $todayMonth]);
    ...
    if ($data && count($data) > 0) {
        ...
        foreach ($data as $warga) {
            ...
            $tglObj = date_create_from_format('d/m/Y', $warga['tgl_lahir']);
            if (!$tglObj) $tglObj = date_create($warga['tgl_lahir']);
            if ($tglObj) {
