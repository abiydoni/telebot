...
try {
    ...
    // 2. Ambil semua data users
    $stmtKK = $pdo->prepare("SELECT uniqueCode AS code_id, name AS kk_name FROM users WHERE statusHubungan = 'Kepala Keluarga' AND villageId = 'village_001' ORDER BY name ASC");
    $stmtKK->execute();
    ...
    foreach ($wargaList as $warga) {
        ...
        $stmtScan = $pdo->prepare("SELECT SUM(amount) as total_scanned, COUNT(id) as scan_count FROM jimpitan_history WHERE kkId = :code_id AND date LIKE :prev_month AND villageId = 'village_001'");
        $stmtScan->execute([
            ':code_id' => $codeId,
